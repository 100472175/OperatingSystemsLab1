...
if (__name__ == "__main__"):
    ...
    if not len(sys.argv) == 2:
        ...
        if len(tokens) != 3 and len(tokens) != 4 and len(tokens) != 5:
            print("Incorrect file name format: os_p1_AAAAA_BBBBB_CCCCC.zip")
            sys.exit(0)
        ...
        subprocess.call(
            "echo \"XDG_DATA_DIRS=/usr/local/share:/usr/share:/var/lib/snapd/desktop\" >> " + tempFolder + "env.txt",
            shell=True)
        subprocess.call(
            "echo \"PATH=/usr/local/bin:/usr/bin:/bin:/usr/local/games:/usr/games:/snap/bin\" >> " + tempFolder + "env.txt",
            shell=True)
        subprocess.call("echo \"DBUS_SESSION_BUS_ADDRESS=unix:path=/run/user/1234/bus\" >> " + tempFolder + "env.txt",
                        shell=True)
        ...
        # Comprobamos que el fichero de authors existe
        if not os.path.isfile(tempFolder + "authors.txt"):
            print("The file authors.txt does not exist")
            sys.exit(0)
